...
GCS_PATH=gs://anaml-tpcds/scale-1-csv
...
gsutil ls $GCS_PATH | while read gcs_file ; do
    ...
    bq --project_id $BQ_PROJECT load --source_format=CSV --field_delimiter '|' --ignore_unknown_values $BQ_DATASET.$TABLE_NAME $GCS_PATH/$FILE_NAME
done
